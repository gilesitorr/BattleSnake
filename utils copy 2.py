...
def score_board(board, snakes, foods, my_snake, alpha=None, beta=None, delta=1, gamma=1, health=100):
    ...
    for snake in snakes:
        ...
        if id == my_snake["id"]:
            continue
        if alpha is None:
            alpha_tmp = 1+len(my_snake["body"])/len(snake["body"])
        else:
            alpha_tmp = alpha
...
def get_next_move(game_state):
    ...
    head = my_snake["head"]
    board_score = score_board(board, snakes, foods, my_snake, alpha=None, beta=None, delta=1, gamma=1, health=my_snake["health"])
